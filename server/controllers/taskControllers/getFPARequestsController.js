  ...
  let promise = new Promise((resolve, reject) => {
    ...
    async function getFPARequests() {
      try {
        ...
        const cursor =  retailerRequests.find({
          userId: req.query.userId,
        },{
          requestName: 1, status: 1, minSupport: 1,_id:0,
          minConfidence: 1, discountStatus: 1, selectedList:1
        });
        await cursor.forEach(document=>{
          fpaRequests.push({requestName: document.requestName,
